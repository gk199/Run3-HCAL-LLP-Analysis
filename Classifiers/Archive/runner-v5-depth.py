...
def _process_signal_file(root_fp, out_h5, tree_name, filter_name, step_size):
    ...
    if os.path.exists(out_h5):
        print(f"Skipping signal file (h5 already exists): {out_h5}")
        return 0
    ...
    with h5py.File(out_h5, "w") as h5f:
        ...
        for chunk in uproot.iterate(f"{root_fp}:{tree_name}", filter_name=filter_name, library="pd", step_size=step_size):
            ...
            keep = lambda df: [c for c in df.columns if c.startswith("perJet") or c.startswith("Pass")]
...
def _process_background_file(root_fp, out_h5, tree_name, filter_name, step_size):
    ...
    import uproot, h5py, gc, pandas as pd
    if os.path.exists(out_h5):
        print(f"Skipping background file (h5 already exists): {out_h5}")
        return 0
...
class DataProcessor:
    ...
    def apply_selections(self, inclusive=False):
        ...
        sig_df = pd.DataFrame()
        combined_h5_path = os.path.join(h5_output_dir, "combined_selected.h5")
