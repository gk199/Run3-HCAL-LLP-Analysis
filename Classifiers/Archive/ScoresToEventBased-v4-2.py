...
class Runner:
    ...
    def evaluate_scores(self): # this code used to be in run_file_evaluation -- still testing
        ...
        handler = ModelHandler(num_classes=self.num_classes, model_name="depth_model_v4_LLPanywhere.keras")
        print("Loading the depth model: inclusive_model_v4_train40.keras")
        handler.load()
        preds = [ handler.predict(predicting_data[i], labels[i]) for i in range(num_jets) ]
        # inclusive
        handler_inc = ModelHandler(num_classes=self.num_classes, model_name="inclusive_model_v4_train80.keras")
        print("Loading the inclusive model: inclusive_model_v4_train80.keras")
        handler_inc.load()
