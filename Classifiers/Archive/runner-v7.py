...
class DataProcessor:
    ...
    def load_data(self, sig_files=None, bkg_files=None, step_size=100000):
        ...
        include_patterns = ["perJet_*", "era_category", "Pass_WPlusJets", "Pass_HLTDisplacedJet", "Pass_InclTagCand", "Pass_DepthTagCand"]
    ...
    def apply_selections(self, inclusive=False):
        ...
        def classify_sig_inclusive(row):
            if select_safety(row) and inclusive_signal(row):
                return 0
            else:
                return -1
        
        classify_sig = classify_sig_inclusive 

        classify_bkg = classify_bkg_inclusive 
...
class ModelHandler:
    ...
    def train(self, X_train, y_train, epochs=200, batch_size=512, val=0.25):
        ...
        name = self.model_name.replace('.keras', '_checkpoint.keras')
...
def main():
    ...
    print("Running Inclusive Tagger - Era categories 1+2")
    runner = Runner(sig_files=sig_files[:], bkg_files=bkg_files[:], mode=mode, num_classes=2, inclusive=True, era_filter=[1, 2])
    runner.set_model_name(model_name="inclusive_model_v7_cat12.keras")
    runner.run()

    print("Running Inclusive Tagger - Era category 3")
    runner = Runner(sig_files=sig_files[:], bkg_files=bkg_files[:], mode=mode, num_classes=2, inclusive=True, load=False, era_filter=[3])
    runner.set_model_name(model_name="inclusive_model_v7_cat3.keras")
    runner.run()
