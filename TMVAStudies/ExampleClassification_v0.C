...
float radius_ECAL[2]     = {129, 177}; // 22cm of ECAL
...
int runClassification( TString dir, TString sigTag )
{
   ...
   TCut selections_all = "jet0_Pt > 40 && abs(jet0_Eta) < 1.26 && jet0_isTruthMatched == 1 && jet0_isMatchedTo == 0";
   TCut selections_safety = "jet0_EtaSpread > 0 && jet0_HadNeutralOverCharged > 0 && jet0_HadNeutralOverCharged < 1e9 && jet0_EnergyFrac_Depth1 > 0 && jet0_EnergyFrac_Depth1 < 1 && jet0_Track1Pt > 0";
   TCut selections_Prompt = LLP0inCR.Data();
   TCut selections_HB     = LLP0inHCAL.Data();

   TCut selections_background = selections_all + selections_safety + selections_Prompt;
   TCut selections_signal = selections_all + selections_safety + selections_HB;
   ...
   TTree *background_temp = (TTree*)bkginput->Get("NoSel");
   ...
   dataloader->AddVariable( "jet0_HadNeutralOverCharged", "jet0_HadNeutralOverCharged", "", 'F' );
