...
public :
	...
	void StampCuts( float x=0.14, float y=0.84, float textsize=0.03, TString stamp_string="") {
		...
		if( stamp_string == "" )
			...
			stamp_text.DrawLatex( x, y, Form( MiniTupleVersion+" %s -- "+GetBetterCutTitle(cuts_all)+"&&"+stamp_string, treename.c_str() ) );
	...
	THStack* GetStackHist( map<string,TH1F*> hists, PlotParams myPlotParams ){ 
		...
		for( auto hist_tag: hist_tags ){
			...
			if( stamp_counts ){
				legend_name = Form("%s (NE=%.f,Int=%.2f,Mean=%.2f)", legend_name.c_str(), h->GetEntries(), h->Integral(), h->GetMean() );
				//legend_name = Form("%s (Mean=%.2f)", legend_name.c_str(), h->GetMean());
			...
			if( plot_log ){ 
				h->SetMaximum( 10 ); //h->GetMaximum()*20. ); // KILEY
				h->SetMinimum( 1e-3 ); //h->GetMaximum()*20. );
			}
			else {
				...
				for( auto hist_tag: hist_tags ){
					...
					if ((multiple == false && i > 0) || (multiple == true && i%2 == 1)) {
						...
						if(TEfficiency::CheckConsistency(*h_pass, *h_total)) {
							...
							if (plot_type == "efficiency" || plot_type == "efficiencyHLT") pEff->Write( output_file_name); //Form("%s_%s", output_file_name, temp.substr(6,3).c_str())); // for L1 efficiency with 2 mass points
	...
	void Draw2DPlot( PlotParams myPlotParams_x, PlotParams myPlotParams_y, string filetag_treename, TCut cut_compare, int i , TCut cut_base = "") {
		...
		//StampCMS( "Internal", 140., 0.14, 0.84, 0.045 );
		StampCuts( 0.12, 0.91, 0.02, GetBetterCutTitle(cut_compare));
		StampText( 0.7, 0.91, 0.04, WriteSelection);
        ...
        myCanvas->SaveAs( Form( output_directory+"/Plot2D_"+output_file_name+"_Cut%d"+saveas_name(0,24)+"_%s.png", i, output_file_tag.c_str() ) );
